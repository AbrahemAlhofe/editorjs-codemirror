 ...
 *
 * @author Abrahem Alhofe (alhofeabrahem@gmail.com)
 * @copyright Abrahem Alhofe 2020
 * @license MIT
 * @version 1.0.0
 */
...
class CodemirrorTool {
  ...
  constructor({ data, config, api }) {
    ...
    this.placeholder = this.api.i18n.t(config.placeholder || CodemirrorTool.DEFAULT_PLACEHOLDER);
    ...
    this.data = {
      code: data.code || '',
      lang : data.lang || this.config.defaultMode
    };
  ...
  drawView() {
    ...
    mode.classList.add(this.CSS.mode);
    wrapper.classList.add(this.CSS.baseClass, this.CSS.wrapper);
    textarea.classList.add(this.CSS.textarea, this.CSS.input);

    mode.innerText = this.data.lang;

    wrapper.appendChild(mode);
    wrapper.appendChild(textarea);

    this.nodes.editor = CodeMirror.fromTextArea(textarea, {
    ...
    mode.addEventListener('dblclick', () => {
      mode.setAttribute('contenteditable', 'true');
    });

    mode.addEventListener('keydown', (e) => {
      if (e.keyCode === 13) {
        e.preventDefault();
        mode.setAttribute('contenteditable', 'false');
        this.nodes.editor.setOption('mode', mode.innerText)
      }
    });
  ...
  set data(data) {
    ...
    if (this.nodes.editor) {
      this.nodes.editor.setValue(data.code);
      this.nodes.editor.setOption('mode', data.lang)
    }
  }
